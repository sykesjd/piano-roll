...
function drawNote(value, part, type, style, tp, noteHeight) {
    ...
    switch (type) {
        ...
        case 10: $note = $('<div></div>').addClass('note').addClass('sp12').css({"left":(x-5),"top":y+'vh'}).append($('<div></div>').addClass('spu').css({'border-bottom-width':noteHeight/2+'vh'})).append($('<div></div>').addClass('spd').css({'border-top-width':noteHeight/2+'vh'}));
    }
    if (value['motifs']) {
